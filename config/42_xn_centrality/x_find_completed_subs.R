...
sink.dirs <- file.path("/home2/data/Projects/ABIDE_Initiative/Derivatives/CCS/Cent", sprintf("pipeline_%s", strats))

csubjs  <- list.files(sink.dirs[1], pattern="session_1")
subjs   <- sub("_session_1", "", csubjs)
n       <- length(subjs)

suffix <- "centrality_outputs_smoothed/_mask_mask_ccs_90percent_gm/_scan_rest_1/_fwhm_6/_network_centrality_smooth_01/degree_centrality_weighted_maths_maths.nii.gz"
