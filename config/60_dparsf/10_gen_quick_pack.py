...
#    "mean_functional": "rest_pp_mean.nii.gz",      # VMHC?
    "functional_brain_mask": "AutoMasks/AutoMasks_NativeSpace/AutoMask_%(subject)s.nii", # YES 
    "functional_nuisance_residuals": "FunImgAR%(strategy)sC/%(subject)s/*.nii",  # YES (although not really used)
    "functional_freq_filtered": "FunImgAR%(strategy)sC%(pipeline)s/%(subject)s/*.nii",    # YES
    "functional_mni": "FunImgAR%(strategy)sCW%(pipeline)s/%(subject)s/*.nii",      # YES
    "functional_brain_mask_to_standard": "AutoMasks/AutoMasks_MNISpace/wAutoMask_%(subject)s.nii" # Yes
}
...
base_dir        = "/home2/data/Projects/ABIDE_Initiative/DPARSF/ReNormalize"
pipelines       = { "nofilt": "", "filt": "F" }
strategies      = { "noglobal": "", 
                    "global":  "global" }
...
def build_file_path(init_dir, path_suffixes, templ):
    ...
    if file_path == "":
        print "\nWARNING: file_path is empty", [op.join(init_dir, path_suffix % templ) for path_suffix in path_suffixes], "\n"
...
for orig_sub_resources in sub_dict:     # SUBJECT (MEAT)
    ...
    sub_dir = glob(op.join(base_dir, "FunImgARC/%s/*.nii" % sub_resources['subject_id']))
    if len(sub_dir) == 1:
        sub_dir = sub_dir[0]
    else:
        print "ERROR: %s doesn't exist for sample directory" % sub_resources['subject_id']
            ...
            for orig_sub_resources in filt_sub_dict:     # SUBJECT (MEAT)
                ...
                for key,path_suffixes in func_suffixes.iteritems():
                    if key not in sub_resources:
                        sub_resources[key] = {}
                    sub_resources[key][scan] = build_file_path(base_dir, path_suffixes, templ)
