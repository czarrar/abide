...
templ = {}
for si,scan in enumerate(scans):                    # SCAN (assume just one scan)
    print "scan", scan
    templ["scan"] = scan
    
    #templ["pipeline"] = "pipeline_%s" % pipelines.values()[0]
            ...
            for orig_sub_resources in sub_dict:     # SUBJECT (MEAT)
                ...
                for key,path_suffixes in func_suffixes.iteritems():
                    if key not in sub_resources:
                        sub_resources[key] = {}
                    sub_resources[key][lab] = build_file_path(sub_dir, path_suffixes, templ)
...
# DOUBLE CHECK
#tmp = yaml.load(open('quick_pack_rest_1_rest_filt_global.yml'))
#bad_subs = {}
#for sub in tmp:
#    bad_sub = {}
...
#                if sv == "":
#                    bad_sub.setdefault(k, {})
#                    bad_sub[k][sk] = v
#    if bad_sub:
#        bad_subs[sub['subject_id']] = bad_sub
